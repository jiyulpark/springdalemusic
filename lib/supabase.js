...
export async function executeQuery(query, maxRetries = 3, isInitialConnection = false) {
  let lastError = null;
  const timeout = isInitialConnection ? 30000 : 10000; // 초기 연결 시 30초, 그 외 10초
  ...
  for (let i = 0; i < maxRetries; i++) {
    try {
      ...
      // 연결 상태 확인
      const { data: healthCheck, error: healthError } = await supabase.from('health_check').select('status').limit(1);
      if (healthError) {
        console.warn('연결 상태 확인 실패:', healthError.message);
      }
...
export async function createSignedUrl(bucket, filePath, expiresIn = 60) {
  ...
  // 파일 경로 정리
  const cleanPath = filePath?.replace(/^uploads\//, '').replace(/^thumbnails\//, '');
  ...
  for (let i = 0; i < maxRetries; i++) {
    try {
      ...
      const { data: fileExists, error: checkError } = await Promise.race([
        ...
        new Promise((_, reject) =>
          setTimeout(() => reject(new Error('파일 존재 확인 타임아웃')), 5000)
        )
      ]);
      ...
      const { data, error } = await Promise.race([
        ...
        new Promise((_, reject) =>
          setTimeout(() => reject(new Error('URL 생성 타임아웃')), 5000)
        )
      ]);
