// pages/api/download.ts
import type { NextApiRequest, NextApiResponse } from 'next';
import { supabase } from '../../lib/supabase'; // 경로 주의!

// 역할 레벨 설정 (낮을수록 낮은 권한)
const roleLevels = {
  guest: 0,
  user: 1,
  verified_user: 2,
  admin: 3,
};

export default async function handler(req: NextApiRequest, res: NextApiResponse) {
  const { postId, filePath } = req.query;

  // 요청 유효성 검사
  if (!postId || !filePath || typeof postId !== 'string' || typeof filePath !== 'string') {
    return res.status(400).json({ error: '잘못된 요청입니다.' });
  }

  try {
    // 세션 확인
    const {
      data: { session },
    } = await supabase.auth.getSession();

    const user = session?.user || null;
    const userId = user?.id || null;

    // 게시글 권한 정보 가져오기
    const { data: post, error: postError } = await supabase
      .from('posts')
      .select('download_permission')
      .eq('id', postId)
      .single();

    if (postError || !post) {
      console.error('❌ 게시글 조회 실패:', postError?.message);
      return res.status(404).json({ error: '게시글을 찾을 수 없습니다.' });
    }

    const requiredRole = post.download_permission || 'guest';

    // 유저 권한 가져오기
    let userRole = 'guest';
    if (userId) {
      const { data: userInfo, error: userError } = await supabase
        .from('users')
        .select('role')
        .eq('id', userId)
        .single();

      if (userError) {
        console.error('❌ 사용자 권한 조회 실패:', userError.message);
      }

      if (userInfo?.role) userRole = userInfo.role;
    }

    // 권한 비교
    if (roleLevels[userRole] < roleLevels[requiredRole]) {
      console.warn(`⛔ 권한 부족: [${userRole}] → [${requiredRole}] 필요`);
      return res.status(403).json({ error: '다운로드 권한이 없습니다.' });
    }

    // 파일 URL 생성
    const { data: storageData, error: storageError } = supabase.storage
      .from('uploads')
      .getPublicUrl(filePath);

    if (storageError || !storageData?.publicUrl) {
      console.error('❌ 파일 URL 생성 실패:', storageError?.message);
      return res.status(500).json({ error: '파일 URL 생성 실패' });
    }

    // 다운로드 수 증가
    const { error: rpcError } = await supabase.rpc('increment_downloads', {
      post_id_input: postId,
    });

    if (rpcError) {
      console.error('❌ 다운로드 수 증가 실패:', rpcError.message);
      return res.status(500).json({ error: '다운로드 수 업데이트 실패' });
    }

    // 파일로 리디렉션
    return res.redirect(storageData.publicUrl);
  } catch (err: any) {
    console.error('❌ 다운로드 API 실패:', err.message || err);
    return res.status(500).json({ error: '서버 오류 발생' });
  }
}
